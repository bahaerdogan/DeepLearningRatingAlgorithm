...
void cosineSimilarity(std::unordered_map<int, std::map<int, User>>& map, int user1Id, int user2Id) {
    ...
    //similarityMap[user2Id].push_back({ user1Id, similarity });
}
...
void predictRating(int userId, int itemId, std::unordered_map<int, std::map<int, User>>& userMap, std::unordered_map<int, std::vector<Sim>>& similarityMap, std::unordered_map<int, std::unordered_map<int, strItem>>& testMap)
{
    ...
    // Define a comparison function that compares the ages of two Person objects
    ...
    //auto comp = [](const Sim& p1, const Sim& p2) { return p1.similarity < p2.similarity; };

    //// Get an iterator to the maximum element
    //auto max_it = std::max_element(sameItem.begin(), sameItem.end(), comp);

    //// Dereference the iterator to get the maximum element
    //Sim max_person = *max_it;

    //actualId = max_person.secondUserId;
    //sim = max_person.similarity;




    predict = sumOfPredicts / count;
    //predict = userMap[actualId][itemId].rating;
    file_id = testMap[userId][itemId].fileId;
    //predictedMap[userId].push_back({ itemId ,predict });
...
int main()
{
        ...
        while (!inputFile.eof())
        {
            ...
            if (userId != "" && itemId != "" && rating != "")
            {
                ...
                else
                {
                    ...
                    itemMap[nUserId].push_back({ nItemId });
    ...
    for (const auto& m : unordered_userMap)
    {
        ...
        /*std::cout << m.first << " "<< userMap[m.first][0].sum << std::endl;*/
    }
    ...
    for (size_t i = 0; i < 10; ++i)
    {
        std::cout << userSum[i].id << " " << userSum[i].sum << std::endl;
    }*/

    /* std::cout << "Top 10 users with the biggest total ratings:" << std::endl;
     for (size_t i = 0; i < 10; ++i)
     {
         std::cout << movieSum[i].id << " " << movieSum[i].sum << std::endl;
     }*/
     ...
     //{
     //    auto& id1 = std::next(testMap.begin(), i)->first;
     //
     //
     //
     //    for (size_t j = 0; j < userMap.size(); j++)
     //    {
     //        auto& id2 = std::next(userMap.begin(), j)->first;
     //
     //        if (id1!=id2)
     //        {
    ...
    for (auto it1 = submissionMap.begin(); it1 != submissionMap.end(); it1++)
    {
        ...
        firstOutput = 0;
        /*std::cout << file_id << "," << vec.rating << std::endl;*/
